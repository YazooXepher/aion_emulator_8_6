...
public class XmlMerger {
	...
	private static class TimeCheckerHandler extends DefaultHandler {
		...
		private boolean checkFile(File file) {
			...
			try {
				...
				logger.warn("File verification error. File: " + file.getPath() + ", location=" + locator.getLineNumber() + ":" + locator.getColumnNumber(), e);
				return true;
			}
	...
	private Properties restoreFileModifications(File file) {
		...
		try {
			...
			logger.debug("File modifications restoring error. ", e);
			return null;
		} finally {
