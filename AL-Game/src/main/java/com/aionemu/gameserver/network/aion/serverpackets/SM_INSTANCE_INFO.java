...
public class SM_INSTANCE_INFO extends AionServerPacket {
	...
	protected void writeImpl(AionConnection con) {
		...
		if (cooldownId == 0) {
			...
			for (InstanceCooltime cooltime : DataManager.INSTANCE_COOLTIME_DATA.getAllInstances().values()) {
				writeD(cooltime.getId());
				writeD(0x0);
				if (cooldownList.getPortalCooldown(cooltime.getWorldId()) == 0) {
					writeD(0x0);
				} else {
					writeD((int) (cooldownList.getPortalCooldown(cooltime.getWorldId()) - System.currentTimeMillis()) / 1000);
				}
				writeD(DataManager.INSTANCE_COOLTIME_DATA.getInstanceEntranceCountByWorldId(cooltime.getWorldId()));
				writeD(cooldownList.getPortalCooldownItem(cooltime.getWorldId()) != null ? cooldownList.getPortalCooldownItem(cooltime.getWorldId()).getEntryCount() * -1 : 0);
				writeD(0);
				writeD(0);
				writeD(0);
				writeD(0);
