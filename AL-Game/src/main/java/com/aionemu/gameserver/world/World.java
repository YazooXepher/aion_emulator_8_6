...
public class World {
	...
	public void storeObject(VisibleObject object) {
		...
		if (object.getSpawn() instanceof BaseSpawnTemplate) {
			...
			baseNpc.computeIfAbsent(baseId, k -> new ArrayList<>()).add((Npc) object);
		}
	...
	public void removeObject(VisibleObject object) {
		...
		if (object.getSpawn() instanceof BaseSpawnTemplate) {
			BaseSpawnTemplate bst = (BaseSpawnTemplate) object.getSpawn();
			int baseId = bst.getId();
			List<Npc> npcs = baseNpc.get(baseId);
			if (npcs != null) {
				npcs.remove(object);
			}
		}
	...
	public void updatePosition(VisibleObject object, float newX, float newY, float newZ, byte newHeading, boolean updateKnownList) {
		...
		if (oldRegion == null) {
			log.warn(String.format("CHECKPOINT: %sId:%d oldRegion is null, map - %d, object coordinates - %f %f %f",
					object.getClass().getSimpleName().replace(".class", ""),
		...
		if (newRegion == null) {
			log.warn(String.format("CHECKPOINT: %sId:%d newRegion is null, map - %d, object coordinates - %f %f %f",
					object.getClass().getSimpleName().replace(".class", ""),
					npc != null ? npc.getNpcId() : 0,
