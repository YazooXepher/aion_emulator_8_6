...
public class Base<BL extends BaseLocation> {
	...
	private void delayedAssault() {
		...
		}, Rnd.get(BaseConfig.ASSAULT_MIN_DELAY, BaseConfig.ASSAULT_MAX_DELAY) * 60000);
	}
	...
	private void delayedSpawn(final Race race) {
		...
		}, Rnd.get(BaseConfig.BOSS_SPAWN_MIN_DELAY, BaseConfig.BOSS_SPAWN_MAX_DELAY) * 60000);
	}
	...
	public boolean isOnlyBalaur(int id) {
		switch (id) {
			case 90: case 91: case 134: case 135: case 136: case 137: case 138: case 139:
	...
	protected void despawn() {
		...
		List<Npc> spawned = World.getInstance().getBaseSpawns(getId());
		if (spawned != null) {
			for (Npc npc : spawned) {
