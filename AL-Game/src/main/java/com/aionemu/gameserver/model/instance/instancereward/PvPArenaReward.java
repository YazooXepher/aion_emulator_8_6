...
public class PvPArenaReward extends InstanceReward<PvPArenaPlayerReward> {
	...
	public List<PvPArenaPlayerReward> sortPoints() {
		List<PvPArenaPlayerReward> rewards = new ArrayList<>(getInstanceRewards());
		Collections.sort(rewards, new Comparator<PvPArenaPlayerReward>() {
			@Override
	...
	public boolean hasCapPoints() {
		if (isSoloArena() && (Collections.max(getInstanceRewards(), Comparator.comparingInt(PvPArenaPlayerReward::getPoints)).getPoints() -
				Collections.min(getInstanceRewards(), Comparator.comparingInt(PvPArenaPlayerReward::getPoints)).getPoints() >= 1500)) {
			return true;
		}
		return Collections.max(getInstanceRewards(), Comparator.comparingInt(PvPArenaPlayerReward::getPoints)).getPoints() >= capPoints;
	}
