		...
		if (mainWeapon != null) {
			...
			if (!functions.isEmpty()) {
				if (getName() == StatEnum.PVP_ATTACK_RATIO || (getName() == StatEnum.PVP_DEFEND_RATIO)) {
					for (StatFunction func : functions) {
						func.apply(stat);
					}
				} else {
					StatFunction maxFunc = Collections.max(functions, new Comparator<StatFunction>() {
						@Override
						public int compare(StatFunction o1, StatFunction o2) {
